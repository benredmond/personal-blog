...
// ABOUTME: Renders markdown content with Neo-Bauhaus L-Frame styling and navigation
...
export async function generateMetadata({ params }: BlogPostPageProps): Promise<Metadata> {
  ...
  try {
    const response = await fetch(`http://localhost:3000/api/blog/posts?page=1&limit=100`, {
      cache: 'no-store',
    });
    ...
    const post: Post | undefined = data.posts?.find((p: Post) => p.slug === slug);
      ...
      <MasterGrid>
        <article className="col-span-12 md:col-span-8 md:col-start-3 relative py-12">
          ...
          <div className="flex flex-wrap items-center gap-4 mb-6 text-sm">
            ...
            {post.published_date && (
              <span className="text-gray-600">{formatDate(post.published_date)}</span>
            )}
            {post.read_time && <span className="text-gray-600">{post.read_time}</span>}
          </div>
